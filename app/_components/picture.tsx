...
import { Typography, Box, Dialog, IconButton, CircularProgress } from '@mui/material'
...
export function PicturesGrid({ pictures, albumId }: { pictures: Picture[], albumId: number }) {
    const { album, albumErrors, albumLoading } = useAlbum(String(albumId))
    const [selectedPicture, setSelectedPicture] = useState<Picture | null>(null)
    const [selectedPictureToDelete, setSelectedPictureToDelete] = useState<Picture | null>(null)
    ...
    const handleImageDelete = async () => {
        ...
        const res = await fetch(`/api/my/albums/${String(selectedPictureToDelete?.albumId)}/pictures/${String(selectedPictureToDelete?.id)}`, {
            method: 'DELETE',
        })
        if (!res.ok) {
